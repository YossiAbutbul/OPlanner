 ...
 */
export const initializeYear = async (year) => {
    try {
 ...
 */
export const initializeYearIfEmpty = async () => {
    try {
 ...
 */
export const getAllYearsAndSemesters = async () => {
    try {
        const yearsCollection = collection(db, "years");
        const snapshot = await getDocs(yearsCollection);
        const yearsData = await Promise.all(snapshot.docs.map(async (yearDoc) => {
            ...
            const semesters = await Promise.all(semestersSnapshot.docs.map(async (semesterDoc) => {
                ...
                const coursesCollection = collection(db, `years/${yearDoc.id}/semesters/${semesterDoc.id}/courses`);
 ...
 */
export const addCourse = async (year, semester, course) => {
    try {
 ...
 */
export const renameCourse = async (year, semester, oldName, newName) => {
    try {
        const coursesCollection = collection(db, `years/${year}/semesters/${semester}/courses`);
 ...
 */
export const deleteCourse = async (year, semester, course) => {
    try {
